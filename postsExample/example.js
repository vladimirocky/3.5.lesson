...
function sendRequest(){
  showCurrentStep('1 - отправляем запрос' );
   ...
   fetch('https://jsonplaceholder.typicode.com/posts')
    ...
    .then(sop => boots(sop))
  /**
   * Советую тут немножко по-другому делать
   * Как только мы получили посты:
   *  1. сразу собрать небольшой массив id пользователей постов
   *  2. и отдельной функцией для каждого id формировать отдельный запрос
   * вида: usersURL + '/' + id
   *
   * Тут мы запросили вообще всех пользователей, а потом их отсеиваем, при этом в boots забираем id постов только
   */
